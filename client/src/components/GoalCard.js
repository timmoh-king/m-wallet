...
const GoalCard = ({ goaltitle, date, targetAmt, savedAmt, walletId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("https://m-wallet.onrender.com/api/stk/", { amount }, config);
      const response = await axios.put(`https://m-wallet.onrender.com/api/get_wallets/${walletId}`, { amount: amount }, config);
